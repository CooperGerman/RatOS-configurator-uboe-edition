...
class ProbeCommandHelper:
    def __init__(self, config, probe, query_endstop=None):
        ...
        gcode.register_command('Z_OFFSET_QUERY_PROBE', self.cmd_QUERY_PROBE,
                               desc=self.cmd_QUERY_PROBE_help)
        # PROBE command
        self.last_z_result = 0.
        gcode.register_command('Z_OFFSET_PROBE', self.cmd_PROBE,
                               desc=self.cmd_PROBE_help)
        # PROBE_CALIBRATE command
        self.probe_calibrate_z = 0.
        gcode.register_command('Z_OFFSET_PROBE_CALIBRATE', self.cmd_PROBE_CALIBRATE,
...
class HomingViaProbeHelper:
    def __init__(self, config, mcu_probe):
        ...
        self.printer.lookup_object('pins').register_chip('z_offset_probe', self)
...
class ProbePointsHelper:
    ...
    def start_probe(self, gcmd):
        manual_probe.verify_no_manual_probe(self.printer)
        # Lookup objects
        probe = self.printer.lookup_object('z_offset_probe', None)
